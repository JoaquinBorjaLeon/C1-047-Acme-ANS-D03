...
public class CustomerBookingRecordPublishService extends AbstractGuiService<Customer, BookingRecord> {
	...
	public void validate(final BookingRecord BookingRecord) {
		boolean publishedBooking = true;
		boolean publishedPassenger = true;
		Booking booking = BookingRecord.getBooking();
		if (booking != null && booking.isDraftMode()) {
			publishedBooking = false;
			super.state(publishedBooking, "booking", "acme.validation.booking.invalid-booking-BookingRecord-publish.message");
		}
		Passenger passenger = BookingRecord.getPassenger();
		if (passenger != null && passenger.isDraftMode()) {
			publishedPassenger = false;
			super.state(publishedPassenger, "passenger", "acme.validation.booking.invalid-passenger-BookingRecord-publish.message");
		}
	}
